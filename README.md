...
worded so a model reports the action as _not_ performed. See
...
For local development, one script runs the API, Conduit Vox, and the Operator
Console together, and stops the trio on Ctrl-C:

```sh
scripts/dev.sh
```

That serves the API anonymously on `127.0.0.1:8080`, Vox on
`127.0.0.1:8091`, and the console on `127.0.0.1:5173`, with the console's
`/v1` proxy pointed at whichever API port is in use and `/vox/*` pointed at the
local Vox service. Real providers come from saved Provider Definitions, as they
do in any other deployment. The flags cover the ports and the two axes that
change:
...
scripts/dev.sh --vox-port 8191                # move the local Vox listener
...
```json
{
  ...
  "management": [{ "token": "…", "name": "ui" }]
}
```

Each device entry names its device, which is what makes an authenticated
conversation know _which_ satellite it is talking to. That name reaches the logs,
...
An unrecognised token gets the _same_ message as a missing one on purpose: a
...
What a device token is _not_ is a speaker. It proves which satellite is
...
What is bounded is _silence_, not length: every event a turn publishes counts as
...
A missing or unrunnable pipeline is refused with an HTTP status _before_ the
...
| ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `conduit_time_to_first_audio_seconds`                       | How long before the assistant _started_ speaking — the latency a person actually feels                                                                               |
...
Earlier versions labelled _every_ turn that lost its listener `barge_in`,
...
A configuration describes one _server_, not one capability, so a host serving
...
the _credential_. Nobody types a Bedrock key. A definition names a region — the
...
`conduit-script` is the one provider whose configuration is a _program_. The
...
_definition is saved_, so a typo is refused while it is still on screen. The
...
| ------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------- |
| [`conduit-elevenlabs`](crates/conduit-elevenlabs) | `stt`, `tts` | The credential is an `xi-api-key` header and the voice is a URL _path segment_              |
| [`conduit-deepgram`](crates/conduit-deepgram)     | `tts`        | `Authorization: Token`, not `Bearer`; the voice _is_ the model, in the query                |
...
the definition is _saved_, so an operator on a host with no credentials is told
...
_not_ as a security boundary — it is a query parameter rather than a path segment,
...
_is_ configured is a region, and optionally a voice and an engine. The engine is a
...
the window the wrong _length of sound_ and the model reports confidently about
...
samples: it decides _which_ chunks the recognizer hears, so what it forwards is
...
differ _about_ that an operator would choose between — what differs is accuracy in
...
backends are two _retrievals_ rather than two places to put the same records.
...
something a graph can currently express _or_ validate, let alone run. It is an
idea, not a pending implementation.

**Barge-in is not detected, only requested.** A client can say `stop`, and that
turn is cancelled as `user_requested`. What no one does is _notice_ someone
...
detector and a `vad` stage — but it runs on the _input_ path, trimming silence
...
_not_ run — because anything ambiguous reads to a model as permission granted,
