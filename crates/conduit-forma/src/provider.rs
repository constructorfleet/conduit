...
use conduit_provider::{Descriptor, Provider, Capability};
...
impl FormaProvider {
    ...
    async fn transform_with_rules(&self, segment: &str) -> Result<String> {
        let rules = if let Some(rule_set_id) = &self.rule_set_id {
            self.store.get_rule_set(rule_set_id).await
                .map_err(|e: FormaError| conduit_core::Error::Config(format!("Forma storage error: {e}")))?
                .map(|set| set.rules)
                .unwrap_or_default()
        } else {
            Vec::new()
        ...
        self.engine.apply_rules(segment, &rules)
            .map_err(|e| conduit_core::Error::Config(format!("Forma transformation failed: {e}")))
    }
}
...
mod tests {
    ...
    async fn transforms_text_using_rules() {
        ...
        let provider = FormaProvider::with_storage("test-forma", store)
            .with_rule_set("test-set");
    ...
    async fn applies_multiple_rules_in_priority_order() {
        ...
        let rule2 = crate::rule::FormaRule::new("rule2", "Rule 2")
            ...
            .with_action(RuleAction::ConvertCase {
                case: crate::rule::CaseConversion::Upper,
            })
            .with_enabled(true)
            .with_priority(5);
