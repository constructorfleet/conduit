...
use crate::rule::{FormaRule, RuleAction, RuleCondition, CaseConversion};
...
impl Engine {
    ...
    fn check_condition(&self, text: &str, condition: &RuleCondition) -> Result<bool, FormaError> {
        match condition {
            ...
            RuleCondition::Contains { substring } => {
                Ok(text.contains(substring))
            }
            
            RuleCondition::StartsWith { prefix } => {
                Ok(text.starts_with(prefix))
            }
            
            RuleCondition::EndsWith { suffix } => {
                ...
                match case {
                    ...
                    CaseConversion::Title => {
                        let result = text
                            ...
                            .enumerate()
                            .map(|(i, c)| {
                                if i == 0 || text.chars().nth(i - 1).map_or(false, |prev| prev.is_whitespace()) {
                    ...
                    CaseConversion::Sentence => {
                        let result = text
                            ...
                            .enumerate()
                            .map(|(i, c)| {
                                if i == 0 || text.chars().nth(i - 1).map_or(false, |prev| prev == '.') {
                                    c.to_uppercase().collect::<String>()
                                } else {
                                    c.to_lowercase().collect::<String>()
                                }
...
mod tests {
    ...
    fn converts_case() {
        ...
        let rule = FormaRule::new("test", "Test Rule")
            ...
            .with_condition(RuleCondition::Always)
            .with_action(RuleAction::ConvertCase {
    ...
    fn applies_conditional_rule() {
        ...
        let rule = FormaRule::new("test", "Test Rule")
            ...
            .with_type(RuleType::Replace)
            .with_condition(RuleCondition::Contains {
                substring: "special".to_string(),
            })
            .with_action(RuleAction::Replace {
